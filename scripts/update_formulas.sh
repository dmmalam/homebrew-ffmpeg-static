...
errors=0
if ! update_formula "Formula/ffmpeg-static.rb" "Release" "${release_version}"
then
  errors=$((errors + 1))
fi
if ! update_formula "Formula/ffmpeg-static-snapshot.rb" "Snapshot" "${snapshot_version}"
then
  errors=$((errors + 1))
fi
