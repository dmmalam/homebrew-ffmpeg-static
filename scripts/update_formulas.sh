...
html=$(curl -s "$BASE_URL") || { echo "ERROR: Failed to fetch website" >&2; exit 1; }
...
[ -z "$snapshot_version" ] && { echo "ERROR: Could not extract snapshot version" >&2; exit 1; }
[ -z "$release_version" ] && { echo "ERROR: Could not extract release version" >&2; exit 1; }
...
update_formula() {
    ...
    idx=0
    for p in "macos|arm64|macOS (Apple Silicon/arm64)" "macos|amd64|macOS (Intel/amd64)" "linux|arm64|Linux (arm64v8)" "linux|amd64|Linux (amd64)"; do
        os=$(echo "$p" | cut -d'|' -f1)
    ...
    [ $errors -gt 0 ] && { echo "  Updated with $errors errors"; return 1; }
...
[ $errors -gt 0 ] && { echo "Completed with errors!"; exit 1; }
echo "Done!"