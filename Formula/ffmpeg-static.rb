...
class FfmpegStatic < Formula
  ...
  def caveats
    <<~EOS
      ...
      To build from source instead, reinstall with:
        brew install --build-from-source ffmpeg-static
    EOS
  end
  ...
  option "with-build-from-source", "Build FFmpeg from source using Martin Riedl's build script"
